...
DESCRIPTION="Generic initramfs generation tool (C99 dracut-cpio fork)"
HOMEPAGE="https://github.com/AntiqueH/dracut"
...
LICENSE="GPL-2"
SLOT="0"
IUSE="dracut-cpio rust selinux systemd test"
RESTRICT="test"
PROPERTIES="test? ( test_privileged test_network )"

REQUIRED_USE="rust? ( dracut-cpio )"
...
PATCHES=(
	"${FILESDIR}"/gentoo-ldconfig-paths-r2.patch
)

pkg_setup() {
	use dracut-cpio && use rust && rust_pkg_setup
}
...
src_configure() {
	...
	if use dracut-cpio && ! use rust; then
		myconf+=( --with-dracut-cpio-c )
	else
		myconf+=( --disable-dracut-cpio )
	fi
...
src_install() {
	...
	# dracut locates the helper with "command -v dracut-cpio", so it has to
	# land somewhere on PATH. The C99 build is installed by "make install"
	# already; only the cargo-built one needs placing by hand.
	if use dracut-cpio && use rust; then
		dobin "src/dracut-cpio/$(cargo_target_dir)/dracut-cpio"
	fi
...
pkg_postinst() {
	if use dracut-cpio; then
		if use rust; then
			elog "dracut-cpio was built from the Rust implementation."
			elog "Set USE=-rust to build the C99 one instead, which needs no Rust toolchain."
		else
			elog "dracut-cpio was built from the C99 implementation."
		fi
		elog "Pass --enhanced-cpio to dracut to make use of it."
	fi
